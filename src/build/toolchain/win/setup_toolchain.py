...
def AppendUniquePath(paths, path):
  if os.path.exists(os.path.realpath(path)) and path not in paths:
    paths.append(path)
...
def main():
  ...
  for cpu in cpus:
    if cpu == target_cpu:
      ...
      msvc_root = os.path.normpath(os.path.join(vc_bin_dir, '..', '..', '..'))
      atl_include = os.path.join(msvc_root, 'atlmfc', 'include')
      atl_lib = os.path.join(msvc_root, 'atlmfc', 'lib', cpu)
      AppendUniquePath(include, relflag(atl_include))
      AppendUniquePath(lib, relflag(atl_lib))
